...
def verif_conf_file(param_dict):
    '''
    Checks the operation parameters of the system information server.
    Replaces invalid values ​​with default values.
    '''
    if 'DataBase' in param_dict:
        if (type(param_dict['DataBase']) is not int or
            (param_dict['DataBase'] != 1 and
             param_dict['DataBase'] != 0)):
            param_dict['DataBase'] = 1
    else:
        param_dict['DataBase'] = 1
    if 'W_Interval' in param_dict:
        if (type(param_dict['W_Interval']) is not int or
                param_dict['W_Interval']) <= 0:
            param_dict['W_Interval'] = 30
    else:
        param_dict['W_Interval'] = 30
    if 'Storage_time' in param_dict:
        if (type(param_dict['Storage_time']) is not int or
                param_dict['Storage_time'] <= 0):
            param_dict['Storage_time'] = 7
    else:
        param_dict['Storage_time'] = 7
    if 'Port' in param_dict:
        if (type(param_dict['Port']) is not int or
                (param_dict['Port'] <= 1024 or param_dict['Port'] > 65535)):
            param_dict['Port'] = 8083
            logger.error(f"The value of the 'port' parameter is incorrect. \
                            Default value set: 8083.")
    else:
        param_dict['Port'] = 8083
        logger.error(f"The value of the 'port' parameter is incorrect. \
                        Default value set: 8083.")
...
def get_param():
    '''
    Reads the configuration file srv_info.conf and returns a dictionary
    with the parameters of the system information server.
    '''
    param = {}
    try:
