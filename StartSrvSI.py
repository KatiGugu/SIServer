'''
Main module. Starts an HTTP server and in a parallel process runs a code that accumulates 
statistical information about the system in the database.
Understands two types of Get request. If the request is of the form http://hostname/current 
then the current information will be returned. If http:/hostname/stat/?time1=t1&time2=t2, 
then records from the database in the "data" range between t1 and t2 will be returned 
...
def CreatHTTPserv(dBase, hostName, serverPort):       
    ...
    InsServerInfo = HTTPServer((hostName, serverPort), InstServerInfoRequestHandler)
...
if __name__ == '__main__':
    ...
    if dic_param['DataBase']:
        dbInfo = dbase()
        process_save_si = Process(target=save_sysinfo, args=(dic_param['W_Interval'], dic_param['Storage_time'], dbInfo))
    ...
    HTTPserv = CreatHTTPserv(dbInfo, hostName = dic_param['HostName'], serverPort = dic_param['Port'])
    try:
        print("Server started http://%s:%s" % (dic_param['HostName'], dic_param['Port']))
